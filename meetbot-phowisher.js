// meet-recorder-dom-audio-http.js
...
const TRANSCRIBE_URL = 'http://46.60.19.166:9000/transcribe?task=transcribe&language=vi';
...
// ====== Call HTTP ASR ======
async function transcribeViaHttp(wavPath, outJsonPath, outTxtPath) {
  log('Transcribing via HTTP:', TRANSCRIBE_URL);

  // Đọc WAV vào buffer rồi bọc thành Blob
  const buf = fs.readFileSync(wavPath);
  const blob = new Blob([buf], { type: 'audio/wav' });

  const form = new FormData();
  // Tham số 3 là filename
  ...
  fs.writeFileSync(outJsonPath, JSON.stringify(data, null, 2), 'utf-8');
  fs.writeFileSync(outTxtPath, (data.text || '').trim(), 'utf-8');

  log('Transcribe done ->', outJsonPath, 'and', outTxtPath);
}
...
async function startAudioDomRecording(page, recordSeconds, audioRawPath, audioWavPath) {
  ...
  await page.evaluate(async (seconds) => {
    ...
    let mediaEls = pickMediaEls();
    if (!mediaEls.length) {
      for (let i = 0; i < 8 && !mediaEls.length; i++) {
        await new Promise(r => setTimeout(r, 1000));
        mediaEls = pickMediaEls();
      }
    }
    ...
    proc.onaudioprocess = async (ev) => {
      ...
      try {
        const i16 = f32ToI16LEBytes(outF32); // PCM16LE 16k mono
...
async function recordGoogleMeet(meetingUrl, botName, recordSeconds = RECORD_SECONDS_DEFAULT) {
  ...
  const browser = await chromium.launch({
    headless: false,
    args: [
      // Không dùng fake media flags để có âm thật
  ...
  try {
    ...
    const transJson = path.join(OUTPUT_DIR, `transcript-${ts}.json`);
    const transTxt = path.join(OUTPUT_DIR, `transcript-${ts}.txt`);
    ...
    // Gọi HTTP ASR và lưu transcript
    await transcribeViaHttp(audioWav, transJson, transTxt);
